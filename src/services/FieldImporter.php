...
class FieldImporter extends Component
{
    ...
    public function getMockRelations(Element $element, array $attributes = null, $mockDataSettings)
    {
        ...
        if (!$results) {
            SproutBaseImport::info(Craft::t('sprout-import', 'Unable to generate mock {displayName} relations. No relationships found: {elementClass}', [
        ...
        if ($relatedMin > $relatedMax) {
            SproutBaseImport::info(Craft::t('sprout-import', 'Unable to generate Mock Data for relations field: {fieldName}. The minimum amount of relations are less than the total available relations. Make sure your mock settings are valid and that you have content available to relate to all required relations fields.', [
                'fieldName' => $fieldName
            ]));
            return false;
        }
