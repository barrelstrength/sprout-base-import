...
class User extends ElementImporter
{
    ...
    private function generateUser($groupId)
    {
        ...
        $username = SproutBaseImport::$app->fieldImporter->generateUsernameOrEmail($username, $faker);

        $email = $faker->email;

        $email = SproutBaseImport::$app->fieldImporter->generateUsernameOrEmail($email, $faker, true);
        ...
        $data['content']['fields'] = SproutBaseImport::$app->fieldImporter->getFieldsWithMockData($fieldLayoutFieldModel);
